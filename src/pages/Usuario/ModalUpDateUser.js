...
export default function MaxWidthDialog({slodin,lodin, dados}) {
  const classes = useStyles();
  const { user } = useContext(Context);

  const [open, setOpen] = React.useState(false);

  const [email, semail ] = React.useState(dados.email);
  const [fistname, setfistName] = React.useState(dados.fistname);
  const [lastname, setlastName] = React.useState(dados.lastname);
  const [status, setstatus] = React.useState(dados.id_status);
  const [tipouser, settipouser] = React.useState(dados.id_tipouser);
  const [capacituser, setcapacituser] = React.useState(dados.capacidade || 0);
  ...
  function getchecked() {
    try {
      dados.apps.map(servicos => {
        const teste =  servicos.servicos.map(servico => {
  ...
  return (
    <React.Fragment>
      ...
      <Tooltip title="Adicionar um usuário" aria-label="edit" size="small" placement="right">
        <Fab className={classes.palette} color="primary" size="small" onClick={handleClickOpen} aria-label="add">
          <EditIcon />
        </Fab>
      </Tooltip>
